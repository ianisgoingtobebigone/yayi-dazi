...
def validate_consent(value: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    retention_days = value.get("retention_notice_days")
    if isinstance(retention_days, bool) or not isinstance(retention_days, int) or not 1 <= retention_days <= 3650:
        raise M08ContractError("retention_notice_days must be an integer from 1 to 3650")
    return {
        ...
        "retention_notice_days": retention_days,
    }
...
def run_m08(
    *,
    ...
    if safety["effective_level"] in HALT_LEVELS:
        return {
            "schema_version": "cn-dental-m08.output.v1.2",
            "module": "M08",
