...
def _validate_route(value: Mapping[str, Any] | None, requested_by_module: str) -> dict[str, Any]:
    ...
    if value.get("route_status") != "confirmed":
        raise M10ContractError("M10_REQUIRES_CONFIRMED_M11_ROUTE")
    primary = str(value.get("primary_module", ""))
    secondary = value.get("secondary_module")
    if primary not in SPECIALIST_MODULES:
    ...
    if secondary is not None and secondary not in SPECIALIST_MODULES:
        raise M10ContractError(f"unsupported M11 secondary_module: {secondary}")
    if secondary == primary:
        raise M10ContractError("M11 secondary_module must differ from primary_module")
    ...
    requested_search_modules = value.get("retrieval_modules")
    expected_search_modules = [primary] + ([str(secondary)] if secondary else [])
    if not isinstance(requested_search_modules, list) or not requested_search_modules:
        raise M10ContractError("M11 retrieval_modules must be a non-empty list")
    if requested_search_modules != expected_search_modules:
        raise M10ContractError("M11 retrieval_modules must exactly match primary and secondary modules")
    return {
        "primary_module": primary,
        "secondary_module": secondary,
        "retrieval_modules": expected_search_modules,
...
def retrieve_m10(
    *,
    ...
    if task == "literature_recommendation":
        raise M10ContractError("literature_recommendation requires the verified metadata pipeline")
    if requested_by_module not in SPECIALIST_MODULES:
    ...
    allowed_types_for_task = TASK_ENTRY_TYPE_ALLOWLIST[task]
    if not requested_types <= allowed_types_for_task:
        raise M10ContractError(
            f"M10 entry_types {sorted(requested_types)} are not allowed for task {task}; "
            f"allowed values are {sorted(allowed_types_for_task)}"
        )
    ...
    modules = {"M09"} if task == "treatment_background" else {requested_by_module}
    route_role = "treatment_background" if task == "treatment_background" else (
        "primary" if requested_by_module == route["primary_module"] else "secondary"
    )
    result_limit = 3 if task == "treatment_background" or route["secondary_module"] is None else (
        2 if route_role == "primary" else 1
    )
    all_catalog_candidates = _eligible_chunks(
        store,
        {str(chunk.get("module")) for chunk in store.chunks},
        ALLOWED_ENTRY_TYPES,
        include_pending_for_audit=include_pending_for_audit,
    )
    module_candidates = _eligible_chunks(
        store,
        modules,
        ALLOWED_ENTRY_TYPES,
        include_pending_for_audit=include_pending_for_audit,
    ...
    active_types = set(requested_types)
    fallback_scope = "none"
    if not fused and requested_types != allowed_types_for_task:
        active_types = set(allowed_types_for_task)
        chunks = _eligible_chunks(store, modules, active_types, include_pending_for_audit=include_pending_for_audit)
        lexical = _bm25(query, chunks)[:20]
        semantic = sorted(
            (
                (str(chunk["chunk_id"]), float(semantic_input[str(chunk["chunk_id"])]))
        ...
        fused = _rrf(lexical, semantic, k=60)[:20]
        fallback_scope = "same_module_allowed_entry_types" if fused else "same_module_exhausted_retrieval_gap"
    elif not fused:
        fallback_scope = "same_module_exhausted_retrieval_gap"
    ...
    unauthorized_modules = sorted({str(item["module"]) for item in final} - modules)
    if unauthorized_modules:
        raise M10ContractError(f"M10 route filter leaked unauthorized modules: {unauthorized_modules}")
    ...
    result["query_contract"] = {
        ...
        "route_authorized_modules": list(route["retrieval_modules"]),
        "requested_by_module": requested_by_module,
        "route_role": route_role,
        "requested_entry_types": sorted(requested_types),
        "active_entry_types": sorted(active_types),
        "entry_types": sorted(active_types),
        "audience": audience,
    }
    result["retrieval_results"] = final
    result["retrieval_gaps"] = [] if final else [{
        "relation": "retrieval_gap",
        "query": query,
        "searched_modules": sorted(modules),
        "full_library_fallback_forbidden": True,
        "must_not_fill_with_model_prior_as_source": True,
    }]
    result["retrieval_execution"] = {
        "route_filter_applied": True,
        "isolated_request_module": sorted(modules),
        "catalog_candidates_before_route_filter": len(all_catalog_candidates),
        "module_candidates_after_route_filter": len(module_candidates),
        "entry_type_candidates_after_filter": len(chunks),
        "lexical_candidates": len(lexical),
        "semantic_candidates": len(semantic),
        "fused_candidates": len(fused),
        "returned": len(final),
        "result_limit_for_route_role": result_limit,
        "fallback_scope": fallback_scope,
        "full_library_fallback_used": False,
        "unauthorized_module_result_count": 0,
