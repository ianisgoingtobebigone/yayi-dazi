...
from audit_knowledge_lifecycle import evaluate as audit_knowledge_lifecycle  # noqa: E402
...
def evaluate() -> dict[str, Any]:
    ...
    governance_record = {
        "deployment_mode": "personal_local",
        "data_mode": "real_photo",
        "age_scope": "adult_confirmed",
        "notice_version": "yayi-dazi-privacy.v1",
        "controller_identity": "personal_local_user",
        "processor_identity": "host_platform",
        "purpose_code": "current_consultation_support",
        "data_categories": ["oral_health_text", "intraoral_photo", "derived_health_assessment"],
        "sensitive_processing_consent": True,
        "photo_processing_consent": True,
        "host_or_third_party_disclosure_acknowledged": True,
        "consent_timestamp": "2026-08-17T09:00:00+08:00",
        "retention_policy_id": "no_skill_persistence_plus_host_terms",
        "processing_location_status": "unknown",
        "cross_border_status": "unknown",
        "deletion_capability": "host_managed",
        "secondary_use_consent": False,
        "training_use_consent": False,
    }
    personal_gate = evaluate_data_processing_gate(governance_record)
    public_demo_gate = evaluate_data_processing_gate(
        {**governance_record, "deployment_mode": "public_demo", "data_mode": "real_text"}
    )
    checks["data_governance_allows_declared_personal_use_and_blocks_public_demo_real_data"] = (
        personal_gate["processing_allowed"] is True
        and public_demo_gate["processing_allowed"] is False
        and "public_demo_real_data_forbidden" in public_demo_gate["blockers"]
    )
    ...
    m10 = retrieve_m10(
        ...
        route_result=_route("M02"), entry_types=["DIS", "PRB"], internal_preview=True,
    )
    m10_store = M10Catalog()
    checks["m10_loads_complete_unified_index"] = (
        len(m10_store.records) == 495
        and len(m10_store.chunks) == 495
        and bool(m10["retrieval_results"])
        and all(item["module"] == "M02" for item in m10["retrieval_results"])
        and m10["retrieval_execution"]["route_filter_applied"] is True
        and m10["retrieval_execution"]["full_library_fallback_used"] is False
        and m10["retrieval_execution"]["unauthorized_module_result_count"] == 0
    )
    ...
    checks["m11_orchestrator_runs_with_bundled_adapters"] = (
        m11["status"] == "ready_for_base_model"
        and m11["route"]["primary_module"] == "M02"
        and m11["user_output_plan"]["mode"] == "adaptive_smart_record"
        and m11["user_output_plan"]["smart_record"]["required_section_ids"]
        == ["chief_concern", "key_facts", "preliminary_assessment", "next_step"]
        and m11["model_request"]["schema_version"] == "cn-dental-m11.model-request.v2"
        and m11["state_machine"]["current_state"] == "SMART_RECORD_DRAFT_READY"
        and m11["episode_state"]["flow_state_history"][0]["from_state"] == "START"
    )
    ...
    lifecycle = audit_knowledge_lifecycle(skill_root=SKILL_ROOT)
    checks["knowledge_lifecycle_register_is_current_and_release_versioned"] = (
        lifecycle["status"] == "current"
        and lifecycle["release_version"] == "0.9.0-portfolio"
        and lifecycle["source_count"] == 41
        and lifecycle["catalog_snapshot_count"] == 1
        and lifecycle["release_blocked_by_maintenance"] is False
    )
